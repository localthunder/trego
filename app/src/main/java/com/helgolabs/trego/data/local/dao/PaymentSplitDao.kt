...
interface PaymentSplitDao {
    ...
    @Query("SELECT * FROM payment_splits WHERE user_id = :userId ")
    suspend fun getPaymentSplitsByUser(userId: Int): List<PaymentSplitEntity>
    ...
    """)
    suspend fun updatePaymentSplitSyncStatusByPayment(
        paymentId: Int,
        status: SyncStatus,
        timestamp: String = DateUtils.getCurrentTimestamp()
    )
